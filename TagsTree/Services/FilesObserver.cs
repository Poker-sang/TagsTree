...
public class FilesObserver : FileSystemWatcher
{
    ...
    private static new void Renamed(object sender, RenamedEventArgs e) => _ = App.MainWindow.DispatcherQueue.TryEnqueue
        (
            () => AppContext.FilesChangedList.Add(new(e.FullPath, FileChanged.ChangedType.Rename, e.OldFullPath.GetName()))
        );

    private static new void Deleted(object sender, FileSystemEventArgs e) => _ = App.MainWindow.DispatcherQueue.TryEnqueue
        (
